...
6. **NSGA-II converges only modestly** — this is a finding, not a failure: over the logged mid-run
   reclustering, population-best round energy improves 1.76%, minimum CH energy 3.61%, and only
   cluster-size balance moves substantially (best 2.99 → 1.85, knee 9.19 → 5.58; read from
   `results/validation/nsga2_convergence.csv`). With 5 heads drawn from 100 candidates, random
   initialization already lands near
